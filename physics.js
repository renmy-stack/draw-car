...
// 天井ブロック: 厚み BLOCK_T の閉じた箱。下をくぐるのが正攻法、巨大なタイヤで上に乗って越えるのが裏技
function block(x0, x1, yc, thick) {
  const top = yc - thick;
  return { x0, x1, yc, top, poly: [{ x: x0, y: top }, { x: x0, y: yc }, { x: x1, y: yc }, { x: x1, y: top }, { x: x0, y: top }] };
}
...
function contact(c, b, px, py, j) {
  ...
  for (const tn of c.TUNNELS) {
    if (px < tn.x0 - 40 || px > tn.x1 + 40 || py - T - 4 > tn.yc || py + T + 4 < tn.top) continue;
    const inside = px > tn.x0 && px < tn.x1 && py < tn.yc && py > tn.top;
